# Running Steampipe on ECS with `credential_source = EcsContainer`

A comprehensive guide for setting up and running Steampipe on Amazon ECS Fargate with credential sourcing from the container.
...
> **NOTE**: You can view a sample [CloudFormation stack](sample-cloudfromation-stack/cf-stack.yaml) to set up all these requirements automatically.
...
1. **Connection Problems**:
   ...
   - Check VPC endpoints if in a private subnet

2. **Authentication Issues**:

   - Make sure the environment variable is properly passed to the new user
   - Verify the task role has appropriate permissions
   - Check the trust policy on the target role

3. **Steampipe Errors**:
   - Make sure all dependencies are installed
   - Verify the steampipe user has the correct permissions
...
- [AWS CLI Assume Role Documentation](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-role.html)
